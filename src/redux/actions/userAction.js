import { POST_LOGIN, SET_TOKEN, SET_PHONE, SET_USERNAME, SET_LOGIN, SET_ID,
   SET_AVATAR, SET_COVER_IMAGE, SET_GENDER, SET_BIRTHDAY, SET_LIST_BLOCK } from "./types";
...
export const setListBlockState = blocks => dispatch => {
  dispatch({
    type: SET_LIST_BLOCK,
    payload: blocks
  });
...
export const login = async (data, dispatch) => {
  ...
  if (loginRs.success) {
    ...
    if (showRs.success) {
      ...
      dispatch(setListBlockState(showRs.data.data.blocked_inbox));
...
export async function searchUser({ token, searchInput}) {
  const config = {
    headers: { Authorization: `Bearer ${token}` }
  };
  ...
  return new Promise(async resolve => {
    await axios.post(API_URL + '/users/search', bodyParameters, config)
      .then(response => {
        return resolve({
          success: true,
          data: response.data,
...
export async function blockUser(token, userId) {
  ...
  return new Promise(async resolve => {
    await axios.post(API_URL + '/users/set-block-user', bodyParameters, config)
      .then(response => {
        return resolve({
          success: true,
          data: response.data,
...
export async function updateAvatarAPI(token, img) {
  const config = {
    headers: { Authorization: `Bearer ${token}` }
  };

  let base64Img = await convertBase64(img);
